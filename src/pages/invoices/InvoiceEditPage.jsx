...
function InvoiceEditPage() {
  ...
  const loadInvoice = async () => {
    try {
      ...
      // Mapear servicios a items del formulario
      const items = invoice.servicios.map(servicio => ({
        tipoItem: servicio.producto ? 'producto' : 'servicio',
        tipo: servicio.tipo,
        descripcion: servicio.descripcion,
        cantidad: servicio.cantidad,
        precioUnitario: servicio.precioUnitario,
        producto: servicio.producto?._id || ''
      }))

      // Establecer valores del formulario
      setValue('propietario', invoice.propietario._id)
      setValue('mascota', invoice.mascota._id)
      setValue('descuento', invoice.descuento || 0)
      setValue('impuestos', invoice.porcentajeImpuestos || 19)
      setValue('observaciones', invoice.observaciones || '')
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Editar Factura</h1>
